...
**Qué se manda a ActiveCampaign (y qué no):** contacto (email/teléfono/nombre) + los
custom fields `funnel_name`/UTMs listados en `ac_custom_fields` de `config.json` (IDs
verificados por API contra la cuenta real — hay un juego de campos UTM con guion bajo
al final que es inválido/legacy, no usarlo) + etiquetas (mismo esquema que ya usa Make
en la landing de registro de este lanzamiento, ver `config.json` → `ac_tags`):
- **Todos** los leads: `mau_lead` y `mau-lanz-2609`.
- **Solo quien NO cualifica**, además: `mau-2609-nc` — es el trigger en AC de la
  automatización del correo posterior (contactTag → automation trigger), configurado
  directamente en AC, no desde este código. Por eso el código **no** llama a ninguna
  automatización directamente: si lo hiciera y la etiqueta también la disparase, el
  contacto entraría dos veces (automatización con multientry) y el correo llegaría
  duplicado.

Las respuestas del cuestionario (Q1-Q4, `resultado_gate`) **no** se mandan a AC bajo
ningún concepto: viven solo en Sheets, y de ahí pasan a la Oportunidad de Airtable si
el lead cualifica y agenda, vía la automatización de Make existente (fuera de alcance
de esta landing).

## 3. Config centralizada (sección 6)

Todo lo que cambia de un lanzamiento a otro vive en `site/config/config.json`:
textos y opciones de las preguntas, umbrales del gate, tags de AC, URL de Calendly,
IDs de Sheets. El Apps Script lo lee del propio
...
- El link de Calendly precarga nombre/email/teléfono por URL pero el usuario puede
...
- [ ] Confirmar que al fallar el gate no se muestra el botón de Calendly y sí se
      añade la etiqueta `mau-2609-nc` en AC — `ActiveCampaign.gs`.
- [ ] Confirmar que todos los leads (cualifiquen o no) llevan las etiquetas
      `mau_lead` y `mau-lanz-2609` en AC.
- [ ] Confirmar que al pasar el gate el link de Calendly abre con nombre/email
      precargados, en la misma pestaña — `site/js/main.js::mountCalendlyCta`.
